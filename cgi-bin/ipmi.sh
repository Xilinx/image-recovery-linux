...
# Index for Board Area offset in FRU header
...
read_header() {
	...
	header=($(read_bytes "$eeprom" 0 8)) || return 1
		(( header[0] == 1 )) || return 1

		local sum
		sum="$(checksum header)" || return 1
		(( sum == 0 )) || return 10
	}
...
read_area() {
	...
	offset=$((offset * 8))
	area=($(read_bytes "$eeprom" "$offset" 8)) || return
		(( area[0] == 1 )) || return
		if (( size == 0 )); then
			size=${area[$IPMI_FRU_AREA_HEADER_SIZE_IDX]}
		fi
		area=($(read_bytes "$eeprom" "$offset" $((size * 8)))) || return
			local sum
...
decode_board_area() {
	...
	local idx=0
	local version=${board_bytes[$((idx++))]}
	local area_len=${board_bytes[$((idx++))]}
	local lang_code=${board_bytes[$((idx++))]}
